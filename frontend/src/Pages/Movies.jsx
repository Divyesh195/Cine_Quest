...
function Movies() {
    ...
    const applyFilter = () => {
        if (genre) {
            var x = MoviesData.filter(mov => mov.path == genre)
            setFilterMovies(x)
            if(x.length ==0){
                setMsg(true)
            }else{
    ...
    return (
        <div>
            <p className='text-3xl font-semibold text-center text-gray-600 my-5'>Explore all available and upcoming titles.</p>
            <div>
                <div className='flex flex-wrap justify-start gap-1 sm:my-5 my-1'>
                ...
                <div className='flex flex-wrap gap-5 justify-center 2xl:justify-start '>
                    {msg ? "No movies avilable for this genre" : " "}
                    {filterMovies.map((item, index) => (
                        ...
                        <div onClick={() => navigate(`/booking/${item._id}`)} key={index} className=' bg-gray-100 grow flex w-140 rounded-lg hover:translate-y-[-5px] transition-all duration-400 cursor-pointer'>
                            ...
                            <div className='pl-2 pt-2 relative flex-2'>
                                <p className='sm:text-2xl text-lg font-bold'>{item.name.toUpperCase()}</p>
                                <p className='sm:text-xl text-lg font-semibold mb-2'>{item.part}</p>
                                <p className='sm:text-lg text-md text-primary font-semibold mb-2'>{item.genre}</p>
                                <p className='sm:text-lg text-xs'>{item.about}</p>
